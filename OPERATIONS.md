...
`install.sh` 会把 `COMPOSE_FILE` 写为 `compose.yaml`（本分支固定控制面同机，
不需要角色/profile），因此同样可以直接执行上面的
`docker compose` 命令。跨机升级建议顺序：`control` → `user` / `media` → `edge`。

::: warning 两个安装脚本都会在 .env 里写死 COMPOSE_FILE
本分支只有 `compose.yaml`（控制面同机的 8 个服务），
...
改 `.env` 或 Compose 文件不会改变部署模式；三种形态各占一个 Git 分支。
:::
...
- `YIYI_NODE_TOKEN` 由安装脚本自动生成并持久化；已有外部节点需执行新版节点升级命令以接收该密钥。
...
- [`README.md`](README.md)：本分支形态说明与部署步骤
