...
安装脚本会直接读取 `.env`，并自动下载 Ed25519 公钥，同时**校验它与镜像内置的厂商
公钥一致**；不一致会中止安装。这是为了避免出现"安装成功但全站 403"的部署：
许可证信任根固定在服务镜像内，部署方无法通过替换公钥文件或改指授权地址来绕过。
...
生产环境建议在 `.env` 中锁定镜像版本，避免 `pull` 时被可变标签意外带到新版本：

```dotenv
YIYI_IMAGE_TAG=2026.09.19-033520
```

留空时使用 `latest`。锁定后 `docker compose pull` 只会拉取该版本，
需要升级时显式修改这个值，也便于按版本回退。
