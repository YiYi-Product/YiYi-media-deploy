#!/usr/bin/env bash
#
# YiYi Media 分布式部署（DISTRIBUTED）—— **控制面同机形态** 安装 / 升级脚本。
#
# 本机承载全部控制面服务（frontend/gateway/config/user/media/license-agent +
# postgres/redis）。Storage 与 Play Agent 是**外部工作节点**，不在本机安装：
# 在网页「节点管理」新增节点后用页面给出的一键命令部署到其他机器。
#
# 不按角色拆分，因此不需要 YIYI_DEPLOY_ROLE：本分支固定同机跑全部控制面。
# 需要每个角色各一台机器时，请用 v3-multi-host 分支。
#
# 许可证要求：edition=DISTRIBUTED。单机版许可证会被拒绝。
#
...
validate_role() {
  # 本分支不按角色拆分：控制面固定同机。保留函数以便 future 复用与清晰报错。
  case "$1" in
    single|all-in-one) return 0 ;;
    *)
      echo "本分支只支持控制面同机部署（不按角色拆分）：$1" >&2
      echo "需要每个角色一台机器时，请改用 v3-multi-host 分支。" >&2
      return 1
      ;;
  esac
}
...
# 本分支不需要角色选择。若 .env 里残留旧的角色设置，明确告知已被忽略，
# 避免用户以为改了 YIYI_DEPLOY_ROLE 就能只跑部分服务。
role="$(env_value .env YIYI_DEPLOY_ROLE)"
role="${role:-single}"
if [[ -n "$role" && "$role" != "single" ]]; then
  echo "提示：本分支固定为控制面同机，.env 中的 YIYI_DEPLOY_ROLE=${role} 已忽略。" >&2
  echo "      需要按角色拆分请用 v3-multi-host 分支。" >&2
fi
role="single"
...
# 用 compose profile 决定本机是否启动 postgres / redis。
# 两者互相独立，因此可以用「外部数据库 + 自带 Redis」这类组合。
# 注意：应用服务不带 profile，所以它们始终会被启动。
compose_profiles=()
[[ "$db_mode" == "bundled" ]] && compose_profiles+=("bundled-postgres")
[[ "$redis_mode" == "bundled" ]] && compose_profiles+=("bundled-redis")
if [[ ${#compose_profiles[@]} -eq 0 ]]; then
  set_env_value .env COMPOSE_PROFILES ""
  echo "  数据库与 Redis 均使用外部实例，本机不启动 postgres / redis 容器。"
else
  set_env_value .env COMPOSE_PROFILES "$(IFS=,; echo "${compose_profiles[*]}")"
  echo "  本机自带：${compose_profiles[*]}"
fi
...
if [[ "$role" == "single" ]]; then
  ...
  # Redis 独立判定：bundled 指本机的 redis 容器，external 保留用户填写的外部地址。
  if [[ "$redis_mode" == "bundled" ]]; then
