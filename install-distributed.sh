#!/usr/bin/env bash
#
# YiYi Media 分布式部署（DISTRIBUTED）安装 / 升级脚本。
#
# 适用模式：分布式部署。控制面按 control / user / media / edge 角色拆分到多台机器，
# 存储与播放能力由外部工作节点提供（工作节点不由本脚本安装）。
# 单机版部署（三容器聚合）请使用 install.sh 与 compose.yaml。
#
# control、user、media、edge 是分布式部署**内部**的角色，不是独立部署模式
# （计划 §3.1）。脚本不接受 single 角色：那是上一代的旧口径。
#
# 本文件是原 install.sh 多服务器能力的延续：保留角色选择、join.env 分发、
# 集群中继证书、bundled/external 数据库模式与信任根校验，只把 Compose 文件
# 换成 compose.distributed.yaml。
#
# 用法：配置好 .env 后执行 `sudo ./install-distributed.sh`。脚本不接受位置参数。
set -euo pipefail

DEPLOY_DIR="$(cd "$(dirname "$0")" && pwd)"
cd "$DEPLOY_DIR"

COMPOSE_FILE="$DEPLOY_DIR/compose.distributed.yaml"

if [[ $# -gt 0 ]]; then
  echo "用法：配置 .env 后执行 ./install-distributed.sh" >&2
  echo "单机版部署请使用 ./install.sh" >&2
  exit 2
fi

if [[ ! -f .env ]]; then
  # 默认模板 .env.example 面向单机版三容器形态，分布式部署必须用分布式模板，
  # 否则会得到一份缺少角色与集群变量的配置。
  cp .env.distributed.example .env
  chmod 0600 .env
  echo "已从 .env.distributed.example 生成 $DEPLOY_DIR/.env。" >&2
  echo "请填写 YIYI_DEPLOY_ROLE、YIYI_SERVER_HOST 等必填项后再次运行 ./install-distributed.sh" >&2
  exit 1
fi
...
# 会错误地解析到单机版三容器文件。
set_env_value .env COMPOSE_FILE "compose.distributed.yaml"
